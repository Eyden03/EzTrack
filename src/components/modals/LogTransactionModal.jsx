      ...
      <DrawerContent>
        ...
        <form onSubmit={handleSubmit} className="px-4 pb-6 space-y-4">
          <div className="flex gap-2">
            <button
              type="button"
              onClick={() => setTxType(CONFIG.TX.INCOME)}
              className={`flex-1 py-3 rounded-xl border text-sm font-bold flex items-center justify-center gap-1.5 transition-all ${
                txType === CONFIG.TX.INCOME ? 'bg-blue-50 border-blue-500 text-blue-800' : 'border-gray-200 text-gray-600'
              }`}
            >
            ...
            <button
              type="button"
              onClick={() => setTxType(CONFIG.TX.EXPENSE)}
              className={`flex-1 py-3 rounded-xl border text-sm font-bold flex items-center justify-center gap-1.5 transition-all ${
                txType === CONFIG.TX.EXPENSE ? 'bg-red-50 border-red-400 text-red-700' : 'border-gray-200 text-gray-600'
              }`}
            >
          ...
          {state.tier !== CONFIG.TIERS.SIMULA && (
            <>
              ...
              <div>
                <label className="block text-[11px] font-bold text-gray-400 uppercase tracking-wide mb-1.5">Note</label>
                <input
                  type="text"
                  value={note}
